...
class LivePlotCallback(BaseCallback):
    def __init__(self, verbose=0, window=100, refresh_every=20, title=None, filename="figures/a2c/training_est"):
...
if __name__ == "__main__":
    ...
    if (inner_env.DETERMINISTICO == 0):
        carpeta = os.path.join("salidas", f"resultados_{fecha_hora}_A2C_est_{inner_env.MODO}")
        os.makedirs(carpeta, exist_ok=True)
    else:
        carpeta = os.path.join("salidas", f"resultados_{fecha_hora}_A2C_det_{inner_env.MODO}")
        os.makedirs(carpeta, exist_ok=True)
