...
class LiveRewardPlotter:
    ...
    def close(self, filename="figures/ql/training_est"):
...
def cargar_o_entrenar_modelo(q_table_path):
    train_env = make_train_env()
    # Verificar si existe la tabla Q
    if os.path.exists(q_table_path):
        try:
            print(f"Cargando tabla Q desde {q_table_path}...")
...
if __name__ == "__main__":
    ...
    Q = cargar_o_entrenar_modelo(Q_TABLE_PATH)
    ...
    if (inner_env.DETERMINISTICO == 0):
        carpeta = os.path.join("salidas", f"resultados_{fecha_hora}_QL_est_{inner_env.MODO}")
        os.makedirs(carpeta, exist_ok=True)
    else:
        carpeta = os.path.join("salidas", f"resultados_{fecha_hora}_QL_det_{inner_env.MODO}")
        os.makedirs(carpeta, exist_ok=True)
